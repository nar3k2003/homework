...
function loadDocFaq() {
    ...
    for ( let index in JSON.parse(this.responseText).result) {
        console.log(JSON.parse(this.responseText).result[index], " --------------------------------");
        
        document.getElementById("contentBox").innerHTML +=
        `<div><div class="accordion" onclick="myFunction(this)">`+ JSON.parse(this.responseText).result[index].question_ru 
        +`</div><div class="panel"><p>`+ JSON.parse(this.responseText).result[index].answer_ru 
        +`</p></div></div>`;
        
    }
    
}
xhttp.open("POST", "https://api.cryptoinfo.me/api/get-faqs");
xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
xhttp.send("api_key=eCGo9bZjoxqGZW8h325LA3wlKV0vq01lIQ4w");
// console.log(JSON.parse(this.responseText));
...
function myFunction(params) {
    ...
    var panel = params.nextElementSibling;
